-- upsert_execution_type.sql
-- Handles both INSERT (add) and UPDATE (edit) for execution types from a single file.
...
--   :name         (required) — the execution type name
--   :description  (optional) — the execution type description
...
SELECT 'redirect' AS component,
       '/settings.sql?tab=execution_types&error=Error:+Missing+execution+type+ID' AS link
...
SELECT 'redirect' AS component,
       '/settings.sql?tab=execution_types&error=Execution+type+already+exists' AS link
WHERE COALESCE(:edit_id, '') != ''
  AND CAST(REPLACE(COALESCE(:edit_id, '0'), ',', '') AS INTEGER) != 0
  AND EXISTS (
    SELECT 1 FROM execution_types
