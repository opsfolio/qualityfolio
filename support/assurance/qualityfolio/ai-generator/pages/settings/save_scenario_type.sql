-- upsert_scenario_type.sql
-- Handles both INSERT (add) and UPDATE (edit) for scenario types from a single file.
...
--   :name         (required) — the scenario type name
--   :description  (optional) — the scenario type description
...
SELECT 'redirect' AS component,
       '/settings.sql?tab=scenario_types&error=Error:+Missing+scenario+type+ID' AS link
...
SELECT 'redirect' AS component,
       '/settings.sql?tab=scenario_types&error=Scenario+type+already+exists' AS link
WHERE COALESCE(:edit_id, '') != ''
  AND CAST(REPLACE(COALESCE(:edit_id, '0'), ',', '') AS INTEGER) != 0
  AND EXISTS (
    SELECT 1 FROM scenario_types
...
SELECT 'redirect' AS component,
       '/settings.sql?tab=scenario_types&success=Scenario+type+updated+successfully' AS link
WHERE COALESCE(:edit_id, '') != ''
  AND CAST(REPLACE(COALESCE(:edit_id, '0'), ',', '') AS INTEGER) != 0
