-- upsert_team_member.sql
-- Handles both INSERT (add) and UPDATE (edit) for team members from a single file.
...
--   :full_name    (required) — the member's full name
--   :designation  (optional) — the member's designation
...
SELECT 'redirect' AS component,
       '/settings.sql?tab=team_members&error=Error:+Missing+member+ID' AS link
...
SELECT 'redirect' AS component,
       '/settings.sql?tab=team_members&error=Team+member+already+exists' AS link
WHERE COALESCE(:edit_id, '') != ''
  AND CAST(REPLACE(COALESCE(:edit_id, '0'), ',', '') AS INTEGER) != 0
  AND EXISTS (
...
SELECT
  TRIM(:full_name),
  NULLIF(TRIM(COALESCE(:designation, '')), '')
WHERE COALESCE(:edit_id, '') = ''
  AND NOT EXISTS (
    SELECT 1 FROM team_members
    WHERE LOWER(TRIM(full_name)) = LOWER(TRIM(:full_name))
