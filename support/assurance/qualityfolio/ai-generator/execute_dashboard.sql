...
-- Handles dashboard initialization and command execution
-- Runs from ai-generator/, so use ../ to access qualityfolio root
...
SELECT 'html' AS component,
    ...
    || COALESCE(sqlpage.exec('bash', '-c', 'cd .. && spry rb run qualityfolio.md 2>&1'), 'Command execution started')
    || '<div class="step">▸ Generating Static Assets (spry sp spc)</div>'
    || COALESCE(sqlpage.exec('bash', '-c', 'cd .. && spry sp spc --fs dev-src.auto --destroy-first --conf sqlpage/sqlpage.json --md qualityfolio.md 2>&1'), 'Command execution started')
    || '<div class="step">▸ Starting Dashboard Service (surveilr web-ui)</div>'
    || COALESCE(sqlpage.exec('bash', '-c', 'cd .. && EOH_INSTANCE=1 PORT=9227 surveilr web-ui -d ./resource-surveillance.sqlite.db > /dev/null 2>&1 & sleep 2 && echo "✓ Service started on port 9227"'), 'Starting in background')
