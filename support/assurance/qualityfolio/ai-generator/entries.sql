...
-- Read markdown destination path from DB (set via Settings → Markdown Paths)
-- Falls back to env var, then to 'test-artifacts' as last resort
SET _markdown_dest = COALESCE(
    (SELECT destination_path FROM path_settings WHERE setting_key = 'markdown_destination'),
...
SELECT 'html' AS component,
    ...
    || ' data-markdown-dest="' || REPLACE(COALESCE($_markdown_dest,'test-artifacts'), '"', '&quot;') || '"'
    || '></div>' AS html;
