...
WITH evidence_paths AS (
  SELECT
    ...
    urpe.file_basename,
    CASE 
      WHEN ur.uri LIKE '%/evidence/%' THEN SUBSTR(ur.uri, INSTR(ur.uri, 'evidence/') + 9)
      WHEN ur.uri LIKE '%/test-artifacts/%' THEN SUBSTR(ur.uri, INSTR(ur.uri, 'test-artifacts/') + 15)
    END AS path_after_marker,
    ROW_NUMBER() OVER (
      PARTITION BY ur.uri
  ...
  WHERE (ur.uri LIKE '%/evidence/%' OR ur.uri LIKE '%/test-artifacts/%')
    AND INSTR(
      CASE 
        WHEN ur.uri LIKE '%/evidence/%' THEN SUBSTR(ur.uri, INSTR(ur.uri, 'evidence/') + 9)
        WHEN ur.uri LIKE '%/test-artifacts/%' THEN SUBSTR(ur.uri, INSTR(ur.uri, 'test-artifacts/') + 15)
...
SELECT
  ...
  ep.nature,
  ep.file_basename,
  -- test_case_id: segment before first '/' in path_after_marker
  SUBSTR(ep.path_after_marker, 1, INSTR(ep.path_after_marker, '/') - 1) AS test_case_id,
  -- cycle: segment between first and second '/' in path_after_marker
  CASE
    WHEN INSTR(SUBSTR(ep.path_after_marker, INSTR(ep.path_after_marker, '/') + 1), '/') > 0
    THEN SUBSTR(
      SUBSTR(ep.path_after_marker, INSTR(ep.path_after_marker, '/') + 1),
      1,
      INSTR(SUBSTR(ep.path_after_marker, INSTR(ep.path_after_marker, '/') + 1), '/') - 1
    )
    ELSE SUBSTR(ep.path_after_marker, INSTR(ep.path_after_marker, '/') + 1)
  END AS cycle,
  -- Classify file type
  CASE
